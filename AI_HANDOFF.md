...
- Backend source branch: `community-orders-page-cleanup` (in `workspace.git`, checked out at `.backend-product-publish`)
- Backend commit: `ba85250`
...
- `社群訂單` table has an 操作 column with 編輯 (modal editing 匯款/到貨/下單/搶購狀態 + 備註 via the existing PATCH endpoints, no refactor). No 刪除: `community_orders`/`community_order_items` have no DELETE grant or RPC for `service_role` — adding real delete needs a migration (GRANT DELETE or a new SECURITY DEFINER RPC), not done — checked and reported per instruction instead of self-approving.
