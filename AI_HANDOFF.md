...
- Feature commit: `4136612` (`Bump backend submodule: inline 賣貨便連結 editor, remove popup`).
- Storefront/shared backend Staging: `https://pinkkkuin-staging.vercel.app` → `dpl_LerrhLnNe2VHuyrifk1TSoZogeim`.
- Backend source commit: `6a65bdf`, pushed to `backend-staging` and referenced by `official-next`.
...
- **賣貨便可下單 moved to a dedicated per-出貨申請 action**: each row in the 出貨申請 table gets a "發送賣貨便通知" button, shown only when that row already has a `marketplace_url` — the admin never types a link in here. Click → `POST /api/backend/community/shipment-requests/[id]/notify` (empty body) → `sendCommunityShipmentMarketplaceNotification(shipmentRequestId)` reads nickname/order_ids/marketplace_url **fresh from the DB at send time** (so it always reflects whatever was last saved via the 賣貨便連結 column below, never a stale client value), builds `{{商品清單}}` from its own `order_ids`, sends, and records with `target_id` = the **shipment request id** (not an order id, since this trigger is scoped to one shipment request which may cover several orders for the same customer). The pre-existing "送出 LINE 下單通知" button (a manual completion marker that predates real LINE sending — just a confirm() + status→`completed`) is unrelated and was left untouched.
- **賣貨便連結 is now edited inline, not via popup** (this round): the 出貨申請 table's 賣貨便連結 column (renamed from 賣貨便) shows a text input pre-filled with the current `marketplaceUrl` + a 保存 button, for any row not `completed`/`cancelled` (those keep the old read-only "開啟連結" display, matching how their status is already locked elsewhere in the same row). 保存 client-validates HTTPS, then calls the exact same `updateCommunityShipmentStatus` (same PATCH endpoint, same pending→accepted auto-transition) the removed popup used — only the input method changed. `editCommunityShipmentMarketplace` and both its chained `window.prompt()` calls are gone; there is no prompt/popup/new-window left anywhere in `lily-backend.html`.
