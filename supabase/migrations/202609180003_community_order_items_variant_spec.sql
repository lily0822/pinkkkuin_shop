alter table public.community_order_items
  add column if not exists variant_spec text;

drop function if exists public.backend_list_community_orders(text, integer, integer);
drop function if exists public.lookup_community_orders_by_nickname(text);
drop function if exists public.backend_import_community_orders(text, jsonb);
...
begin
  ...
  loop
    ...
    v_spec := nullif(trim(coalesce(v_row->>'variant_spec', '')), '');
    ...
    insert into public.community_order_items (order_id, import_batch_id, product_name, variant_spec, quantity, unit_price)
    values (v_order_id, v_batch_id, v_product, v_spec, v_qty, v_price);
