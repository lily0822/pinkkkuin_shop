...
import { ArrowRight, CalendarDays, Megaphone, Search, Sparkles, WandSparkles } from "lucide-react";
import { ProductCard } from "@/components/product-card";
import { getStorefrontCategories, getFeaturedStorefrontProducts } from "@/lib/storefront-products";
...
export default async function HomePage() {
  ...
  const quickCategories = [
    ...
    ...storefrontCategories.slice(0, 4).map((category) => ({ label: category, href: `/category/${encodeURIComponent(category)}` })),
  ];
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-12 px-4 py-6">
      ...
      <section className="nav-scrollbar flex gap-2 overflow-x-auto rounded-3xl bg-penguin-pink-light/80 p-3 text-sm font-bold shadow-sm">
      ...
      <section className="grid gap-5 lg:grid-cols-12">
        <div className="lg:col-span-9">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase tracking-widest text-penguin-pink-dark">Penguin Picks</p>
              <h2 className="text-2xl font-black text-penguin-gray">小企鵝推薦選物</h2>
            </div>
            <Link href="/products" className="hidden rounded-full bg-white px-4 py-2 text-sm font-black text-penguin-pink-dark shadow-sm sm:inline-flex">
              查看全部
            </Link>
          </div>
        ...
        <aside className="space-y-5 lg:col-span-3">
          ...
          <div className="rounded-3xl border-2 border-penguin-peach bg-white p-5 shadow-md">
      ...
      <section className="grid gap-4 md:grid-cols-2">
        {announcements.map((announcement) => (
          <article key={announcement.id} className="rounded-3xl border-2 border-penguin-peach bg-white p-5 shadow-sm">
            <div className="flex items-start gap-3">
              <Megaphone className="mt-1 text-penguin-pink-dark" size={20} />
              <div>
                <h2 className="font-black text-penguin-gray">{announcement.title}</h2>
                <p className="mt-2 text-sm leading-7 text-gray-500">{announcement.content}</p>
              </div>
            </div>
          </article>
        ))}
      ...
      <section className="rounded-3xl border-4 border-penguin-pink bg-white p-6 shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="text-xl font-black text-penguin-gray">購物流程</h2>
          </div>
        </div>
        <div className="mt-6 grid gap-3 md:grid-cols-5">
          {["挑選商品", "加入選物箱", "透過 LINE / IG 確認", "完成付款", "等待日本寄回"].map((step, index) => (
